...
lora_list = [{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/Greenery.safetensors",
    "strength":0.9,
    "guidance":7.0,
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/bichu-v0612.safetensors",
    "strength":0.9,
    "guidance":7.0,
    "num_inference_steps":200,
    "image_description": "A man standing in an urbanic setting in front of a lake",
...
},
{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/xrs2.0.safetensors",
    "strength":0.9,
    "guidance":7.0,
    "num_inference_steps":200,
    "image_description": "A man standing in an urbanic setting in front of a lake",
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/cosmic.safetensors",
    "strength":0.9,
    "guidance":0.7,
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/steel.safetensors",
    "strength":0.65,
    "guidance":8.5,
    "num_inference_steps":70,
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/retro.safetensors",
    "strength":0.8,
    "guidance":7,
    "num_inference_steps":70,
    "image_description": "a black man standing in gray shirt and black jeans",
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/ghibli.safetensors",
    "strength":0.9,
    "guidance":5.5,
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/nyalia.safetensors",
    "strength":0.9,
    "guidance":7.0,
    "num_inference_steps":200,
...
},
{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/gta.safetensors",
    "strength":0.9,
    "guidance":7.0,
    "num_inference_steps":200,
    "image_description": "young smiling black boy with curly hair in a blue tshirt.",
...
},
{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/zombie.safetensors",
    "strength":0.9,
    "guidance":6.0,
    "num_inference_steps":200,
    "image_description": "young smiling black boy with curly hair in a blue tshirt.",
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/disney.safetensors",
    "strength":0.75,
    "guidance":7.1,
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/cyberpunk.safetensors",
    "strength":0.75,
    "guidance":6.5,
    "num_inference_steps":100,
    "image_description": "young smiling black boy with curly hair in a blue tshirt",
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/sketch.safetensors",
    "strength":0.99,
    "guidance":6.8,
    "num_inference_steps":100,
    "image_description": "young smiling black boy with curly hair in a blue tshirt",
...

{
    ...
    "path_to_lora":"/workspace/app/style_transfer/sd1.5loras/paint.safetensors",
    "strength":0.8,
    "guidance":8,
    "num_inference_steps":200,
    "image_description": "elephant in a grassland with hills in background and cloudy sky",
...
app = FastAPI(title="LoRA Img2Img API")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
embed_model = SentenceTransformer("all-mpnet-base-v2").to("cuda")
embed_model.eval()

print("Caching LoRA embeddings...")
texts = []
for l in lora_list:
    combined = f"{l['name']} {l['description']} {' '.join(l['trigger_words'])}"
    texts.append(combined)

LORA_EMB = embed_model.encode(texts, convert_to_tensor=True)


def find_best_lora(prompt: str):
    emb = embed_model.encode(prompt, convert_to_tensor=True)
    scores = util.cos_sim(emb, LORA_EMB)[0]
...
class SDModelManager:
    ...
    def load(self):
        if self.pipe is None:
            ...
            print("Pipeline loaded into GPU!")
            return True
        else:
    ...
    def unload(self):
        if self.pipe is not None:
            print("UNLOADING Stable Diffusion Pipeline...")
            del self.pipe
            self.pipe = None
            torch.cuda.empty_cache()
            torch.cuda.ipc_collect()
            print("Pipeline removed from GPU.")
            return True
        return False


model_mgr = SDModelManager()



@app.post("/load_model")
def load_model():
    status = model_mgr.load()
    return {"status": "loaded" if status else "already_loaded"}


@app.post("/unload_model")
def unload_model():
    status = model_mgr.unload()
    return {"status": "unloaded" if status else "not_loaded"}
...
async def generate(
    ...
    if model_mgr.pipe is None:
        print("Auto-loading pipeline for generation...")
        model_mgr.load()
    ...
    best_lora, score = find_best_lora(prompt)
    print("Selected:", best_lora["name"])

    trigger = " ".join(best_lora["trigger_words"])
    final_prompt = f"{trigger} {prompt}"
    final_negative = unify_prompt(best_lora["negative_prompt"])
...
@app.get("/")
def home():
    return {"message": "LoRA Img2Img API with Dynamic Loading is running!"}
