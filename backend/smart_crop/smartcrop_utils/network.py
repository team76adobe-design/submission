...
def vfn_rl(x, variable_dict, global_feature=None, h=None, c=None, embedding_dim=1000):
    ...
    # Note: LRN is deprecated but still available in TF2
    ...
    maxpool1 = tf.nn.max_pool2d(lrn1, ksize=[k_h, k_w], strides=[s_h, s_w], padding=padding)
    ...
    k_h = 3; k_w = 3; s_h = 2; s_w = 2; padding = 'VALID'
    maxpool2 = tf.nn.max_pool2d(lrn2, ksize=[k_h, k_w], strides=[s_h, s_w], padding=padding)
    ...
    k_h = 3; k_w = 3; s_h = 2; s_w = 2; padding = 'VALID'
    maxpool5 = tf.nn.max_pool2d(conv5, ksize=[k_h, k_w], strides=[s_h, s_w], padding=padding)
    # Flatten: Use tf.reshape instead of deprecated tf.contrib.layers.flatten
    bn5 = tf.reshape(maxpool5, [tf.shape(maxpool5)[0], -1])

    ####################### Layer 6 ################################
    fc6W = variable_dict["fc6w"]
    fc6b = variable_dict["fc6b"]
    # Replace tf.nn.relu_layer (deprecated) with explicit operations
    fc6 = tf.nn.relu(tf.matmul(bn5, fc6W) + fc6b)
    ...
    fc1 = tf.nn.relu(tf.matmul(x_rl, fc1W) + fc1b)
    ...
    # tf.multinomial is replaced with tf.random.categorical in TF2
    logits = tf.matmul(h, action1w) + action1b
